...
import { asPlayerID, GUESSER_PLAYER, JUDGE_PLAYER, otherPlayer, PlayerIDType } from "../../common/types";
      ...
      startNewGame: {
        moves: {
          startGame: ({ G, _ctx, playerID, events }) => {
            if (asPlayerID(playerID) !== GUESSER_PLAYER || G.numberOfTry !== 0) {
              return INVALID_MOVE;
            }
            events.endTurn();
          },
      ...
      play: {
        turn: {
          ...
          onEnd: ({G, ctx, _playerID, events}) => {
            if (asPlayerID(ctx.currentPlayer) === JUDGE_PLAYER) {
              const currentTime = new Date();
        ...
        moves: {
          newProblem({ G, _ctx, playerID, events }, problemText: string, nextProblemMaxPoints: number, correctnessPreviousAnswer: boolean, url: string) {
            if (asPlayerID(playerID) !== JUDGE_PLAYER || G.answer === null) {
              // He is not the bot OR G.answer is null (and it is not the first question)
              return INVALID_MOVE;
            }
            G.url = url;
          ...
          submitAnswer({ G, _ctx, playerID, events }, answer: number) {
            if (asPlayerID(playerID) !== GUESSER_PLAYER || !Number.isInteger(answer) || answer < 0 || answer > 9999) {
              return INVALID_MOVE;
            }
            G.answer = answer;
            events.endTurn();
