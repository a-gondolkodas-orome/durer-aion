...
The b) version is no longer deployed by hand. It ships as `/proba-verseny/`
on the one GitHub Pages site that `.github/workflows/pages-deploy.yml` builds
from every push to `main` — there is no `npm run deploy`, no `predeploy`
`PUBLIC_URL` to edit locally, and no `gh-pages` branch to push to. See
[`docs/pages-consolidation.md`](docs/pages-consolidation.md) for what that
deploy contains and how the base paths are assigned, and `npm run site:build`
/ `npm run site:serve` to preview it locally.
