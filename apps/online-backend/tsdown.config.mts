...
// rolldown resolves them through tsconfig.json's `paths` — the same entries
// vitest.config.mts names — so this build needs no `packages/*/dist` and cannot
// go stale against one. The server imports `game` and `game/bot`, never
// `game/client`, so the bundle carries no board and requires neither React nor
// MUI.
...
export default defineConfig({
  ...
  deps: {
    ...
    neverBundle: true,
    // `paths` resolves each of these to a source file, but the external check
    // runs on the bare specifier first, and they all belong to declared
    // dependencies; this is what makes the source win.
